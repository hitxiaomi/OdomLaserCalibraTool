          ...
          std::vector<odometerData> &odom_data, std::vector<laserScanData> &laser_data)
{
  ...
  for (rosbag::MessageInstance const m: view) {
    ...
    //    sensor_msgs::JointStateConstPtr odom =
    //    m.instantiate<sensor_msgs::JointState>(); if (odom != NULL) {
    //      odometerData tmp;
    //      tmp.timestamp = odom->header.stamp;
    ...
    //    }
    //
    // geometry_msgs::Vector3StampedConstPtr odom =
    // m.instantiate<geometry_msgs::Vector3Stamped>(); if (odom != NULL) {
    //   odometerData tmp;
    //   tmp.timestamp = odom->header.stamp;
