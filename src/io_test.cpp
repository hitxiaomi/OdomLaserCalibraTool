...
#include "laser_odom_error_term.h"
...
int main(int argc, char **argv) {
  ...
  dataIO.readDataFromBag(bagfile, laser_topic, odom_topic, odom_data,
                         laser_data);

  std::cout << colouredString("Hi!", WHITE, REGULAR) << std::endl;
  std::cout << colouredString("laser size: ", WHITE, REGULAR)
            << laser_data.size() << '\n'
            << colouredString("odom size: ", WHITE, REGULAR) << odom_data.size()
            << std::endl;
  ...
  std::cout << colouredString("Converting scan to csm type...", YELLOW, REGULAR)
            << std::endl;
  cScan.scanToLDP(laser_data, ldp);
  std::cout << colouredString("Converting finished!", GREEN, REGULAR)
            << std::endl;

  std::cout << colouredString("LDP numbers: ", WHITE, REGULAR) << ldp.size()
            << std::endl;

  std::cout << colouredString("Canonical scan matching...", YELLOW, REGULAR)
            << std::endl;
  cScan.match(laser_data, ldp, scan_odom, match_results);
  std::cout << colouredString("Matching finished!", GREEN, REGULAR)
            << std::endl;

  if (scan_odom.size() == 0) {
    std::cout << colouredString("ERROR! No matching results!", RED, BOLD)
              << std::endl;
  }

  std::cout << colouredString("Writing results...", YELLOW, REGULAR)
            << std::endl;

  std::ofstream fout(resultfile);
  for (int i = 0; i < scan_odom.size(); i++) {
    fout << scan_odom[i].timestamp << ' ' << scan_odom[i].x << ' '
         << scan_odom[i].y << ' ' << scan_odom[i].theta << '\n';
  }
  ...
  std::cout << colouredString("Saving results...", YELLOW, REGULAR)
            << std::endl;
  fout << std::flush;
  fout.close();
  fout3 << std::flush;
  ...
  std::cout << colouredString("Start synchronzing...", YELLOW, REGULAR)
            << std::endl;
  ...
  std::cout << colouredString("Sync results saved!", GREEN, REGULAR)
            << std::endl;

  ceres::Problem problem;
  double l_x = 0.0, l_y = 0.0, l_theta = 0.0, left_wheel_radius = 0.12,
         right_wheel_radius = 0.12, bias = 0.3;
  for (const auto &calibration_data : sync_results) {
    ceres::CostFunction *cost_function =
        LaserOdomErrorTerm::Create(calibration_data);
    problem.AddResidualBlock(cost_function, nullptr, &l_x, &l_y, &l_theta,
                             &left_wheel_radius, &right_wheel_radius, &bias);
  }
  ...
  std::cout << l_x << "  " << l_y << " " << l_theta << "  "<< left_wheel_radius << " "
            << right_wheel_radius << "  " << bias << std::endl;
